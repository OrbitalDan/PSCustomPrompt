function Get-PoshGitRender( $status )
{
  ...
  if ($status -and $s)
  {
    ...
    $retstr += (Set-TerminalColor -F $PSColors[$branchStatusForegroundColor.ToString()].Code) + `
      $branchName
    ...
    {
      $retstr += (Set-TerminalColor -F $PSColors[$s.BeforeIndexForegroundColor.ToString()].Code) + `
        $s.BeforeIndexText

      if($s.ShowStatusWhenZero -or $status.Index.Added)
      {
        $retstr += (Set-TerminalColor -F $PSColors[$s.IndexForegroundColor.ToString()].Code) + `
      ...
      {
        $retstr += (Set-TerminalColor -F $PSColors[$s.DelimForegroundColor.ToString()].Code) + `
          $s.DelimText
      }
    }
    ...
    if($s.EnableFileStatus -and $status.HasWorking)
    {
      ...
      {
        $retstr += (Set-TerminalColor -F $PSColors[$s.WorkingForegroundColor.ToString()].Code) + `
    ...
    {
      $retstr += (Set-TerminalColor -F $PSColors[$localStatusForegroundColor.ToString()].Code) + `
        (" {0}" -f $localStatusSymbol)
    }
    
    if ($s.EnableStashStatus -and ($status.StashCount -gt 0))
    {
      $retstr += (Set-TerminalColor -F $PSColors[$s.BeforeStashForegroundColor.ToString()].Code) + `
        $s.BeforeStashText
      $retstr += (Set-TerminalColor -F $PSColors[$s.StashForegroundColor.ToString()].Code      ) + `
        $status.StashCount
      $retstr += (Set-TerminalColor -F $PSColors[$s.AfterStashForegroundColor.ToString()].Code ) + `
        $s.AfterStashText
    }
